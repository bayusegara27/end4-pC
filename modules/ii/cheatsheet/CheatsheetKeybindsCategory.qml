...
Column {
    ...
    property var keySubstitutions: Object.assign({
      ...
      (Config.options.cheatsheet?.keySubstitutions) || {}
    )
        ...
        Repeater {
            ...
            model: {
                if (!root.isCategorized) {
                    return HyprctlKeybinds.keybinds.filter(bind => root.hasDescription(bind) && root.isUncategorized(bind) && !root.containsNonFirstRepetitive(bind));
                }
                return HyprctlKeybinds.keybinds.filter(bind => root.hasDescription(bind) && root.isCategory(bind, root.categoryName) && !root.containsNonFirstRepetitive(bind));
            }
            delegate: BindLine {
    ...
    component BindLine: Row {
        ...
        Row {
            ...
            Row {
                ...
                Repeater {
                    model: {
                        if (Config.options.cheatsheet && Config.options.cheatsheet.splitButtons) return root.modMaskToStringList(bindLine.keyData.modmask).map(mod => root.keySubstitutions[mod] || mod)
                        const modString = root.modMaskToStringList(bindLine.keyData.modmask).map(mod => root.keySubstitutions[mod] || mod).join(" ")
                        if (modString.length == 0) return []
                        return [modString]
                    }
                    delegate: KeyboardKey {
                        required property var modelData
                        key: root.transformKey(modelData)
                        pixelSize: (Config.options.cheatsheet && Config.options.cheatsheet.fontSize) ? Config.options.cheatsheet.fontSize.key : Appearance.font.pixelSize.normal
                    }
                }
                StyledText {
                    id: keybindPlus
            ...
            Item {
                ...
                StyledText {
                    ...
                    font.pixelSize: (Config.options.cheatsheet && Config.options.cheatsheet.fontSize) ? Config.options.cheatsheet.fontSize.comment : Appearance.font.pixelSize.smaller
